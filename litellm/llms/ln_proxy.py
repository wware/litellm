# litellm/llms/ln_proxy.py - Simplified version avoiding complex imports
import os
import json
import requests
...
# Simple imports that definitely exist
...
class LNProxyProvider:
    ...
    def _extract_tool_calls_simple(self, text: str) -> List[Dict[str, Any]]:
        """Extract tool calls from text and return as simple dicts (avoids import issues)"""
        ...
        if json_str:
            try:
                ...
                if parsed_tool_call.get("tool_call"):
                    # Return as simple dict that can be converted to proper format later
                    return [{
    ...
    def _convert_response(self, response_data: Dict[str, Any], model: str, **kwargs) -> ModelResponse:
        ...
        tool_calls_raw = self._extract_tool_calls_simple(assistant_content)
        
        # Create message dict (avoid complex class imports)
        message_dict = {
            "content": assistant_content if not tool_calls_raw else None,
            "role": "assistant"
        }
        
        # Add tool calls if present (as simple dicts)
        if tool_calls_raw:
            message_dict["tool_calls"] = tool_calls_raw
        
        # Create the message using the Message class
